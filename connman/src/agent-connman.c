...
//    connman_agent_get_info(&dbus_sender, &agent_sender, &agent_path);
	agent = connman_agent_get_info(NULL, &agent_sender,
							&agent_path);

	DBG("agent %p path %s", agent, agent_path);
    if (agent_path == NULL) {
        return -ESRCH;
    }
